...
export default function ManagerPropertiesTab() {
  ...
  }, [currentPage, searchTerm, typeFilter, minPriceFilter, maxPriceFilter]);
  ...
  const fetchProperties = async () => {
    try {
      ...
      const res = await api.get(`/manager/properties?${params.toString()}`);
  ...
  const handleSave = (e) => {
    ...
    showConfirm("Confirm Save", "Are you sure you want to save this property?", async () => {
      ...
      try {
        if (editingId) {
          await api.put(`/manager/properties/${editingId}`, cleanData);
        } else {
          await api.post('/manager/properties', cleanData);
        }
        setShowModal(false);
        fetchProperties();
      } catch (err) {
  ...
  const handleToggleStatus = (prop, newStatus) => {
    showConfirm("Change Status", `Are you sure you want to change the status to ${newStatus}?`, async () => {
      try {
        ...
        await api.put(`/manager/properties/${prop.id}`, updatedProp);
        fetchProperties();
      } catch (err) {
